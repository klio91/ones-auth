...
│   │   │   ├── service.py    생성/승인/비활성화 + Keycloak Role 연동
...
CREATE TABLE users (
    ...
    status       TEXT NOT NULL DEFAULT 'waiting',  -- waiting / active / inactive
...
| 항목 | 값 |
|------|-----|
| Access Token 쿠키명 | `ones_access` |
| Refresh Token 쿠키명 | `ones_refresh` |
| HttpOnly | `true` |
